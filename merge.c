...
void check_rep_cascading_node(cascading_node_t *c){
  ...
  // sentinel
  assert(c->ys[c->size] == INT_MAX);
  int i;
  for (i = 0; i < c->size; i++){
    assert(c->ys[i] < INT_MAX);
  }
  ...
  if (!cascading_node_is_leaf(c)){
	...
	assert( c->right_successor[i] > -1 && c->right_successor[i] >= c->right_successor[i-1]
		&& c->right_successor[i] <= c->right->size);
...
void cascading_node_leaf_init(cascading_node_t *cn, int *x, int *y, int size){
  cn->size = size;
  cn->xs = x;
  cn->ys = (int*) malloc((size+1)*sizeof(int));

  memcpy(cn->ys, y, size*sizeof(int));
  cn->ys[size] = INT_MAX;
...
void cascading_node_parent_init(cascading_node_t *cn, cascading_node_t *left, cascading_node_t *right){
  ...
  cn->ys = (int *)malloc((cn->size+1)*sizeof(int));
  cn->ys[cn->size] = INT_MAX;

  //does not need any sentinel value, simply the other coordinate
  cn->xs = (int *)malloc(cn->size*sizeof(int));
 ...
 */
int* binary_successor_search(int value, int* start, int* end){
...
*/
int arg_successor_root(int yvalue, cascading_node_t *root){
...
void test1(){
 ...
 int expected_ys[] = {100, 200, 300, 400, 500};
 int expected_xs[] = {1, 2, 3, 4, 5};
...
void testBinarySuccessorSearch(){
  ...
  assert(*binary_successor_search(10, a1, a1+4) == INT_MAX);
  assert(*binary_successor_search(100, a1, a1+4) == INT_MAX);
}
